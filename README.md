...
- **SSH-доступ** — чтение логов с удалённых серверов по SFTP/ключу; проверка хостов `accept-new` + `~/.ssh/known_hosts`, файлы стримятся без полной загрузки в память
